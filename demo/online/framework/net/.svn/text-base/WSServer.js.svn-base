      ...
      ws.on('message', (data) => {
        ...
        if (ws && ws._rcvBf && Buffer.isBuffer(data)) {
          ...
          if (msgInfo && Array.isArray(msgInfo.messages) && msgInfo.messages.length > 0) {
            msgInfo.messages.forEach(function (msg) {
              try {
                if (msg && typeof self.tmHandler === 'function') {
                  ...
                  if (typeof msg === 'string') {
                    ...
                    if (bodyLen > 0) {
                      ...
                      logger.error(`msg ${JSON.stringify(strMsg.head)} subLen bigger than total len from ${ws}.`);
                      return;
                    }
                ...
                logger.error(`WSServer on byte message ${msg} catch Error: ${e.message}`);
              }
            });
          }
        }
          ...
          try {
            ...
            logger.error(`WSServer on data message ${data} catch Error: ${e}`);
          }
        }
        else {
...
WSServer.prototype.tmSendData = function (data, timeout) {
  ...
  keys.forEach(function (k) {
    ws = self.clients[k];
    if (ttLen > 8192) logger.warn(`WSServer snd msg total len > 8192: ${ttLen} to ${ws}`);
...
WSServer.prototype.sendWebSocketData = function (ws, data, timeout) {
  ...
  if (ttLen > 8192) logger.debug(`WSServer snd msg total len: ${ttLen} to ${ws}`);
  self._sendWebSocketBuffer(ws, dtBf, timeout);
};
