...
WSClient.prototype.connect = function () {
  ...
  let promise = new Promise(function (resolve, reject) {
    ...
    self.ws.on('error', (error) => {
      ...
      if (typeof self.ws.terminate === 'function') self.ws.terminate();
    });
    ...
    self.ws.on('close', (error, desc) => {
      ...
      if (self.opt.hasOwnProperty('reconnect') && self.opt.rest_connected_times > 0) {
        ...
        logger.warn(`${self.side} ws with remote ${self.ws._strRemote} will be reconnect ${self.opt.rest_connected_times} times in ${tm}ms`);
        setTimeout(() => {
          if (!self.ws) {
          ...
          msgInfo.messages.forEach(function (msg) {
            try {
              if (msg) {
                ...
                else if (self.opt.twoLen) {
                  if (f_debug) logger.debug(msg);
                  let subLen = null;
                  if (self.opt.isLE) {
                    subLen = msg.readUInt32LE(0);
                  }
                  else {
                    subLen = msg.readUInt32BE(0);
                  }

                  let zipped = msg.readUInt8(4);
                  if (zipped) {
                    logger.warn("zipped flag in message, but not support now....");
                  }

                  if (f_debug) logger.debug(`msg buff len:${msgLen}, subLen: ${subLen}`);
                  let headBuf = Buffer.alloc(subLen - 5);
                  msg.copy(headBuf, 0, 5, subLen);

                  strMsg = { head: null, body: Buffer.alloc(0) };
                  strMsg.head = JSON.parse(headBuf.toString('utf8'));

                  const bodyLen = msgLen - subLen;
                  if (bodyLen > 0) {
                    if (strMsg.head.ret !== 0) { // 错误码时包体为 json
                      strMsg.body = Buffer.alloc(bodyLen, 0);
                      msg.copy(strMsg.body, 0, subLen, msgLen);
                      if (strMsg.head.ret < 0) {
                        const errorJson = JSON.parse(strMsg.body.toString('utf8'));
                        strMsg.body = errorJson;
                      }
                    }
                    else {
                      strMsg.body = Buffer.alloc(bodyLen, 0);
                    ...
                    logger.debug(`msg ${JSON.stringify(strMsg.head)} has no body or body is empty.`);
                  }
                  else {
                    logger.error(`msg ${JSON.stringify(strMsg.head)} subLen bigger than total len from ${self.ws}.`);
                    return;
                  }
              ...
              console.log(msg);
              logger.error(`WSClient on message ${msg} catch Error: ${e.message}`);
            }
          });
        }
        else {
...
WSClient.prototype.tmSendData = function (data, timeout) {
  ...
  if (self.opt.twoLen) {
    ...
    let byteHead = null;
    let byteBody = null;
    // ttLen(4) + subLen(4) + zip(1) + jsonHead + protoBody
    let strHead = null;
    if (typeof data.head === 'object' && data.body instanceof Uint8Array) {
      strHead = JSON.stringify(data.head);
      byteHead = Buffer(JSON.stringify(data.head), 'utf8');
      byteBody = Buffer.from(data.body, 'hex');

      subLen = 4 + 1 + byteHead.length;
      bdLen = byteBody.length;
      ttLen = 4 + subLen + bdLen;
    }
    else if (typeof data.head === 'object' && typeof data.body === 'object') {
      strHead = JSON.stringify(data.head);
      byteHead = Buffer(JSON.stringify(data.head), 'utf8');

      subLen = 4 + 1 + byteHead.length;
      const strBody = JSON.stringify(data.body);
      byteBody = Buffer(strBody, 'utf8');
      bdLen = byteBody.length;
      ttLen = 4 + subLen + bdLen;
    }
    else {
      logger.error(`WSServer.sendWebSocketData but parameter data error.`);
      return;
    }

    dtBf = Buffer.alloc(ttLen, 0);
    dtBf.writeUInt32LE(ttLen, 0);
    dtBf.writeUInt32LE(subLen, 4);
    dtBf.writeUInt8(0, 8);  // 压缩标识, 目前不支持压缩实现
    byteHead.copy(dtBf, 9, 0, byteHead.length);
    byteBody.copy(dtBf, 9 + byteHead.length, 0, byteBody.length);
    if (f_debug) logger.debug(`Msg ${strHead}, bufflen:${dtBf.length - 4}, sublen:${subLen}, bodyLen:${bdLen}`);
  }
  else {
    let strData = null;
  ...
  if (ttLen > 8192) logger.warn(`WSServer snd msg total len > 8192: ${ttLen} to ${self.ws}`);
